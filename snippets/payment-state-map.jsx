export const PaymentStateMap = () => {
  ...
  return <div className="psm not-prose">
    <div className="psm-toolbar"><label>Recorrido de ejemplo<select value={scenario} onChange={event => {setScenario(event.target.value);setStep(-1);setPlaying(false);setSelected("registered");}}><option value="settled">Pago autorizado y liquidado</option><option value="reversed">Pago autorizado y extornado</option><option value="denied">Intento denegado</option><option value="expired">Intento expirado</option><option value="canceled">Intento cancelado</option><option value="invalid">Intento inválido</option></select></label><button type="button" onClick={() => { if (reduced) {advance();return;} if (playing) {setPlaying(false);return;} if (step < 0 || step >= routes[scenario].length - 1) {setStep(0);setSelected("registered");} setPlaying(true); }}>{reduced ? "Siguiente paso →" : playing ? "Pausar" : "Reproducir recorrido →"}</button></div>
    <div className="psm-scroll" tabIndex="0" aria-label="Diagrama de estados; desplázate horizontalmente en pantallas pequeñas"><div className="psm-canvas">
      ...
      <span className="psm-stage-label">DESPUÉS DE AUTORIZAR · OPERACIÓN</span>
    </div></div>
    <div className={"psm-detail psm-" + active.tone} aria-live="polite" aria-atomic="true"><strong>{active.name}</strong><p>{active.text}</p></div>
    <p className="psm-caption">Las flechas muestran alternativas, no pasos obligatorios. La expiración y las acciones disponibles dependen del método de pago.</p>
  </div>;
};
