export const MigrationCheckout = ({ locale = "es" }) => {
  ...
  return <div className="mig-checkout-stage mig-interactive ph-no-capture" data-private="true">
    ...
    <div className={"mig-checkout" + (status === "approved" ? " mig-checkout-approved" : "")}>
      <div className="mig-shop"><span className="mig-shop-icon" title="Store illustration: Twemoji (CC BY 4.0)"><img className="mig-store-illustration" src="/images/demo-store-logo.png" width="25" height="25" alt={t("Tienda ilustrada", "Illustrated store")} /></span><div><strong>{t("Tu comercio", "Your store")}</strong><small>{t("Prueba la experiencia", "Try the experience")}</small></div><div className="mig-languages" aria-label={t("Idioma de la demo", "Demo language")}><button type="button" aria-pressed={!en} onClick={() => {setLanguage("es");setError("");}}>ES</button><button type="button" aria-pressed={en} onClick={() => {setLanguage("en");setError("");}}>EN</button></div></div>
