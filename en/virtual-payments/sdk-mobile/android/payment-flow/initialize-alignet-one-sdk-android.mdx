...
<div className="ao-guide-rail ao-sdk-rail sdk-android not-prose"><a href="/en/virtual-payments/sdk-mobile/android/introduction">← Android</a><span>ANDROID / IMPLEMENT</span></div>
...
<Info>
  Before starting SDKcomplete [Installation SDK Android](/en/virtual-payments/sdk-mobile/android/installation) and generates an `Access Token` from your backend using [Authentication](/en/api-reference/authentication/generate-access-token).
</Info>
...
class MainActivity : AppCompatActivity(), PaymeClientDelegate {
    override fun onRespondsPayme(response: PAYMEChargesResponse) {
        // Process the final flow result.
    }

    override fun onPaymeEvents(event: PaymeEvent) {
        // Record SDK navigation or interaction events.
    }
}
```
...
<CardGroup cols={2}>
  ...
  <Card title="RETURN_LIST_PAYMENT_METHODS" icon="arrow-left">
    Indicates that the user returned to the payment method list.
  </Card>

  <Card title="CANCEL_PAYMENT_METHOD" icon="ban">
    Indicates that the user cancelled a payment method.
  </Card>
